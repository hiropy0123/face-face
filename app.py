...
def render_similarity_result(
    result: SimilarityResult, det_a: FaceDetection, det_b: FaceDetection
) -> None:
    ...
    with col_vs:
        st.markdown(
            "<div style='text-align:center;font-size:36px;padding-top:40px'>VS</div>",
    ...
    with col_score:
        st.markdown(
            ...
            <div style="text-align:center;padding:16px 0;">
                ...
                <div style="font-size:13px;color:#888;margin-top:6px;">{level_desc}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    rows = [
        {
            "特徴": fs.name,
            "スコア(%)": round(fs.score, 1),
            "画像Aの値": round(fs.value1, 4),
            "画像Bの値": round(fs.value2, 4),
        }
...
_JP_STATS: list[tuple[str, float, float, str, str]] = [
    ...
    ("目の縦位置",   0.43, 0.03, "> 0.46 = 目が下寄り(幼顔)", "< 0.40 = 目が上寄り(大人顔)"),
    ...
    ("口幅比",       0.43, 0.04, "─",                       "─"),
    ("下顔面比",     0.33, 0.04, "< 0.28 = 短い下顔面(幼顔)", "> 0.38 = 長い下顔面(大人顔)"),
...
def tab_face_type() -> None:
    ...
    metrics            = extract_metrics(landmarks, w_img, h_img)
    ...
    with col_img:
        lm_mode = st.radio(
            "ランドマーク表示",
            options=["非表示", "メッシュ", "ポイント"],
            horizontal=True,
            key="lm_mode",
        )
        mode_map = {"非表示": "none", "メッシュ": "mesh", "ポイント": "points"}
        display_img = draw_landmarks_on_image(image_rgb, landmarks, mode=mode_map[lm_mode])
        st.image(display_img, caption="ランドマーク検出結果", use_column_width=True)

    with col_result:
        kw_html = "　".join(
            f'<span style="background:#374151;color:#d1d5db;padding:3px 10px;'
            f'border-radius:999px;font-size:12px;">{kw}</span>'
        ...
        st.markdown(
            ...
            <div style="padding:20px;border-radius:12px;background:#1a1f2e;text-align:center;">
                ...
                <div style="margin-top:14px;">{kw_html}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        with m_col:
            st.metric(
                "成熟度スコア",
                f"{(metrics.maturity_score + 1) / 2 * 100:.0f}%",
                help="0% = 幼顔 / 100% = 大人顔",
            )
        with l_col:
            st.metric(
                "直線性スコア",
                f"{(metrics.linearity_score + 1) / 2 * 100:.0f}%",
                help="0% = 曲線系 / 100% = 直線系",
            )
    ...
    with chart_left:
        st.plotly_chart(
            create_position_chart(metrics.maturity_score, metrics.linearity_score, face_type),
    ...
    with detail_left:
        with st.expander("計測値の詳細を見る", expanded=False):
            _render_metrics_table(metrics)
    with detail_right:
        st.markdown("#### 8 タイプ早見表")
        st.dataframe(
            pd.DataFrame(
                [{"タイプ": f"{v['emoji']} {k.value}", "軸": v["axis"]}
